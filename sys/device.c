...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, BthMiGamepadCreateDevice)
#pragma alloc_text(PAGE, BthMiGamepadEvtDeviceFileCreate)
#pragma alloc_text(PAGE, BthMiGamepadEvtFileClose)
#endif
    ...
    _Inout_ PWDFDEVICE_INIT DeviceInit)
{
    ...
    pnpPowerCallbacks.EvtDeviceSelfManagedIoInit = BthMiGamepadEvtDeviceSelfManagedIoInit;
    ...
    WDF_FILEOBJECT_CONFIG_INIT(&fileobjectConfig,
                               BthMiGamepadEvtDeviceFileCreate,
                               BthMiGamepadEvtFileClose,
...
NTSTATUS
BthMiGamepadEvtDeviceSelfManagedIoInit(
...
--*/
{
    ...
    status = BthMiGamepadRetrieveLocalInfo(&devCtx->Header);
    if (!NT_SUCCESS(status))
    {
        goto exit;
    }

    status = BthMiGamepadRetrieveServerBthAddress(devCtx);
    if (!NT_SUCCESS(status))
    {
        goto exit;
    }
...
NTSTATUS
BthMiGamepadConnectionStateConnected(
...
VOID BthMiGamepadEvtDeviceFileCreate(
    _In_ WDFDEVICE Device,
...
--*/
{
    ...
    status = BthMiGamepadRetrievePsmFromSdpRecord(&sdpParseInterface,
                                                  serverSdpRecord,
                                                  &fileCtx->ServerPsm);
    ...
    //
    status = BthMiGamepadOpenRemoteConnection(devCtx,
                                              FileObject,
                                              Request);
...
--*/
{
    ...
    // synchronously.
    //
    BthEchoConnectionObjectRemoteDisconnectSynchronously(&(devCtx->Header),
                                                         connection);
}
